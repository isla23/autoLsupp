...
#[0.65,0.58,0.75,0.63]
#[0.63,0.52,0.75,0.58]                       
L_SUPPORT_IMAGE_BOX = [0.1, 0.13, 0.25, 0.19]  # 后勤完成界面判断区域
DESKTOP_IMAGE_BOX = [0.10,0.20,0.22,0.35]#模拟器桌面判断区域         

#=================点击拖动区域=================#

#收后勤支援
L_SUPPORT_STEP1_CLICK_BOX = [0.84,0.79,0.93,0.815]#确认后勤完成
L_SUPPORT_STEP2_CLICK_BOX = [0.53,0.61,0.62,0.64]#再次派出
...
def showWindow():
    ...
    if hwnd == 0 and hwnd_desktop == 0:
        ...
        logger.debug("显示少女前线 - MuMu模拟器！")
        win32gui.ShowWindow(hwnd,win32con.SW_SHOW)
    elif hwnd_desktop != 0:
        logger.debug("显示MuMu模拟器！")
...
def takeLSupport():
    ...
    mouseClick(L_SUPPORT_STEP1_CLICK_BOX,1,1)
    mouseClick(L_SUPPORT_STEP2_CLICK_BOX,1,1)
...
if __name__ == "__main__": 
    ...
    while True:
        ...
        if isMainMenu():
            ...
            takeLSupport()
            L_SupportCount += 1
            logger.debug("收派后勤数："+str(L_SupportCount))
            continue
        elif isDesktop():
            logger.debug("模拟器桌面")
            failCount = 0
